    ...
    <div class="layui-card">
      ...
      <div class="layui-card-body">
        <div style="padding-bottom: 10px;">
          ...
          <button class="layui-btn layuiadmin-btn-useradmin" onclick="xadmin.open('添加用户','${ctx}/jsp/userfrom.jsp?-1',500,350)">添加</button>
        </div>
...
<script>
    ...
    function set(userid){
    	$.post("${ctx}/bookBorrowCardAction!setManage.action", { user_id: userid }, function (data) {
           	layer.msg(data, {
                    icon: 1,
                    time:1000
                }, function () {
	...
	function operation(record) {
		...
		return "<div style='margin:0 -60px;' class='layui-table-cell laytable-cell-2-0-6'> <a style='color:#fff' class='layui-btn layui-btn-normal layui-btn-xs' lay-event='edit' onclick=\"xadmin.open('编辑用户','${ctx}/jsp/userfrom.jsp?" + userid + "',500,400)\"><i class='layui-icon layui-icon-edit'></i>编辑</a>  <a style='color:#fff' class='layui-btn layui-btn-danger layui-btn-xs' onclick='set("+userid+")' lay-event='del'><i class='layui-icon layui-icon-set'></i>设为管理员</a>  </div>";
	}
</script>
